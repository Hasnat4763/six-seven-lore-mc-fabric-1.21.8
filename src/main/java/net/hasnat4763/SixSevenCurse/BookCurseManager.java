...
public class BookCurseManager {
    ...
    public static void triggerTimedEffect(ServerPlayerEntity player, int curseLevel) {
        ...
        switch (curseLevel) {
            case 0 -> triggerLevel0Effect(player, world);
    ...
    private static void triggerLevel0Effect(ServerPlayerEntity player, ServerWorld world) {
        ...
        switch (effect) {
            ...
            case 1 -> world.playSound(null, player.getBlockPos(),
                    SoundEvents.ENTITY_ENDERMAN_AMBIENT, SoundCategory.AMBIENT, 0.5f, 0.5f);
            case 2 -> world.spawnParticles(ParticleTypes.SMOKE,
    ...
    // LEVEL 1: NOTICEABLE (5-15 minutes)
    private static void triggerLevel1Effect(ServerPlayerEntity player, ServerWorld world) {
        ...
        switch (effect) {
            ...
            case 2 -> {
                ...
                Vec3d behindPlayer = player.getPos().subtract(lookVec.multiply(3));

                for (int i = 0; i < 10; i++) {
                    world.spawnParticles(ParticleTypes.SMOKE,
                            behindPlayer.x, behindPlayer.y + 1, behindPlayer.z,
                            1, 0.2, 0.5, 0.2, 0.01);
                }
            }
            case 3 -> player.sendMessage(Text.literal("§k||||§r §4Those who nose§r §k||||"), true);
        }
    }

    // LEVEL 2: UNSETTLING (15-30 minutes)
    private static void triggerLevel2Effect(ServerPlayerEntity player, ServerWorld world) {
        ...
        switch (effect) {
            case 0 -> {
                Vec3d lookVec = player.getRotationVec(1.0f);
                Vec3d behindPlayer = player.getPos().subtract(lookVec.multiply(5));
                BlockPos soundPos = BlockPos.ofFloored(behindPlayer);
            ...
            case 1 -> player.addStatusEffect(new StatusEffectInstance(
                    StatusEffects.BLINDNESS, 10, 0, false, false));
            case 2 -> world.playSound(null, player.getBlockPos(),
            ...
            case 4 -> player.sendMessage(
                    Text.literal("§4§o" + player.getName().getString() + "..."), true);
        }
    }

    // LEVEL 3: SCARY (30-60 minutes)
    private static void triggerLevel3Effect(ServerPlayerEntity player, ServerWorld world) {
        ...
        switch (effect) {
            ...
            case 1 -> {
                for (int i = 0; i < 3; i++) {
                    ...
                    world.getServer().execute(() -> {
                        try {
                            Thread.sleep(step * 400L);
                            world.playSound(null, player.getBlockPos(),
                                    SoundEvents.BLOCK_STONE_STEP, SoundCategory.BLOCKS, 1.5f, 0.7f);
            ...
            case 3 -> {
                Vec3d lookVec = player.getRotationVec(1.0f);
                Vec3d behindPlayer = player.getPos().subtract(lookVec.multiply(8));

                for (int i = 0; i < 30; i++) {
                    world.spawnParticles(ParticleTypes.SMOKE,
                            behindPlayer.x, behindPlayer.y + RANDOM.nextDouble() * 2, behindPlayer.z,
                            1, 0.2, 0, 0.2, 0.01);
                }

                world.playSound(null, BlockPos.ofFloored(behindPlayer),
                        SoundEvents.ENTITY_ENDERMAN_SCREAM, SoundCategory.HOSTILE, 0.5f, 0.5f);
            }
        }
    }

    // LEVEL 4: TERRIFYING (60-120 minutes)
